...
class PurchaseRequestAdmin(admin.ModelAdmin):
    list_display = ['request_id', 'employee_id', 'approval_id', 'valid_date', 'document_date', 'required_date', 'req_type', ]
    list_filter = ['req_type']
    search_fields = ['request_id', 'employee_id__first_name', 'employee_id__last_name', 'approval_id__approval_id']

    fieldsets = (
        (None, {
            'fields': ('request_id', 'employee_id', 'approval_id', 'valid_date', 'document_date', 'required_date', 'req_type')
        }),
    )

class EmployeeAdmin(admin.ModelAdmin):
    list_display = ('employee_id', 'first_name', 'last_name', 'email', 'dept_id')  # Custom fields for display
